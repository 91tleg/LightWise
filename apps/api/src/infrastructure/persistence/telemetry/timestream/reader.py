...
class TimestreamReader:
    ...
    def _build_legacy_single_measure_query(
        ...
        return (
            ...
            f"FILTER (WHERE measure_name IN ('temperature', 'temperature_c')) AS temp_c, "
            f"AVG(measure_value::bigint) "
            f"FILTER (WHERE measure_name IN ('humidity', 'humidity_pct')) AS hum_pct, "
            f"AVG(measure_value::bigint) "
            f"FILTER (WHERE measure_name = 'motion') AS motion, "
            f"AVG(measure_value::bigint) "
            f"FILTER (WHERE measure_name IN ('light_level', 'light_level_pct')) AS light_pct "
            f'FROM "{self._database}"."{self._table}" '
